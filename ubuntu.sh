...
sudo apt install openjdk-8-jdk openjdk-21-jdk -y
...
then
  dpkgInstall "mysql-workbench.deb" $'https://dev.mysql.com/get/Downloads/MySQLGUITools/mysql-workbench_'$root_app_version$'-1_amd64.deb'
...
json="`echo "$json" | jq '."window.titleBarStyle"="native"'`"
json="`echo "$json" | jq '."window.customTitleBarVisibility"="never"'`"
json="`echo "$json" | jq '."window.zoomLevel"=0'`"
json="`echo "$json" | jq '."window.newWindowDimensions"="inherit"'`"
...
json="`echo "$json" | jq '."terminal.integrated.env.linux"."JAVA_HOME"="'$java21_dir'"'`"
json="`echo "$json" | jq '."terminal.integrated.cursorStyle"="underline"'`"
json="`echo "$json" | jq '."terminal.integrated.fontSize"=13'`"
json="`echo "$json" | jq '."terminal.integrated.stickyScroll.enabled"=false'`"
...
json="`echo "$json" | jq '."gitlens.advanced.messages"={}'`"
json="`echo "$json" | jq '."gitlens.advanced.messages"."suppressLineUncommittedWarning"=true'`"
...
json="`echo "$json" | jq '."java.configuration.runtimes"+=[{"name":"JavaSE-21","path":"'$java21_dir'","default":true}]'`"
json="`echo "$json" | jq '."java.jdt.ls.java.home"="'$java21_dir'"'`"
json="`echo "$json" | jq '."spring-boot.ls.java.home"="'$java21_dir'"'`"
